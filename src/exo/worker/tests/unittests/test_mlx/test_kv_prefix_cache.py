...
class TestKVPrefix:
    ...
    def test_v4_logarithmic_snapshot_recovers_92k_of_120k_match(self):
        ...
        query = mx.concatenate(
            [cached_prompt[:92_000], mx.arange(200_000, 200_010, dtype=mx.int32)]
        )
        prefix_cache = KVPrefixCache(None)
        prefix_cache.prompts = [cached_prompt]
        prefix_cache._entry_generations = [1]
        ...
        prefix_cache._snapshots = [
            [
                CacheSnapshot(states=[], token_count=8_192),
                CacheSnapshot(states=[], token_count=16_384),
                CacheSnapshot(states=[], token_count=36_864),
                CacheSnapshot(
                    states=[_make_v4_cache(offset=77_824, pool_rows=19)],
                    token_count=77_824,
                ),
                CacheSnapshot(states=[], token_count=114_688),
                CacheSnapshot(states=[], token_count=118_784),
                CacheSnapshot(states=[], token_count=119_999),
            ]
        ]
        prefix_cache._media_regions = [[]]
        prefix_cache._last_used = [0]
        ...
        assert matched_index == 0
        assert not is_exact
        assert cache_length(restored) == 77_824
        assert mx.array_equal(remaining, query[77_824:])

    def test_exact_shorter_v4_hit_keeps_tokens_for_generation(self):
        cached_prompt = mx.arange(120, dtype=mx.int32)
        ...
        prefix_cache._snapshots = [
            [
                ...
                CacheSnapshot(
                    states=[_make_v4_cache(offset=90, pool_rows=22)],
                    token_count=90,
                ),
                CacheSnapshot(
                    states=[_make_v4_cache(offset=118, pool_rows=29)],
                    token_count=118,
                ),
            ]
        ]
    ...
    def test_v4_update_promotes_tail_snapshot_to_logarithmic_landmark(self):
        ...
        prefix_cache._snapshots = [
            [
                ...
                CacheSnapshot(states=[], token_count=75_776),
                CacheSnapshot(states=[], token_count=77_824),
                CacheSnapshot(states=[], token_count=78_999),
            ]
        ]
        prefix_cache._media_regions = [[]]
        ...
        updated_cache = [_make_v4_cache(offset=80_999, pool_rows=20)]
        prefix_cache.update_kv_cache(
            0,
            mx.arange(81_000, dtype=mx.int32),
            updated_cache,
            [CacheSnapshot(states=updated_cache, token_count=80_999)],
            restore_pos=78_999,
        )

        newer_cache = [_make_v4_cache(offset=89_999, pool_rows=22)]
        prefix_cache.update_kv_cache(
            0,
    ...
    def test_v4_snapshot_plan_uses_absolute_landmarks_after_restore(self):
        assert (
            _v4_snapshot_progress_targets(
                initial_token_count=78_999,
                num_tokens=2_002,
                snapshot_step=2_048,
            )
            == set()
        )
        assert _v4_snapshot_progress_targets(
        ...
        def fake_stream_generate(*, prompt, prompt_cache, prompt_progress_callback, **_):
            ...
            for processed in (
                ...
                prompt_cache[0] = _make_v4_cache(
                    offset=processed,
                    pool_rows=processed // 4096,
                )
                prompt_progress_callback(processed, total)
            prompt_cache[0] = _make_v4_cache(
                offset=total - 1,
                pool_rows=(total - 1) // 4096,
            )
    ...
    def test_prefill_resets_pipeline_flags_after_pipeline_error(self):
        ...
        with (
            ...
            patch(
                "exo.worker.engines.mlx.generator.generate.pipeline_parallel_prefill",
                side_effect=RuntimeError("prefill failed"),
            ),
            patch("exo.worker.engines.mlx.generator.generate.mx_barrier"),
            patch(
                "exo.worker.engines.mlx.generator.generate.set_pipeline_prefill"
            ) as set_prefill,
            patch(
                "exo.worker.engines.mlx.generator.generate.set_pipeline_queue_sends"
            ) as set_queue_sends,
            pytest.raises(RuntimeError, match="prefill failed"),
        ):
            prefill(
                model,
                MagicMock(),
                MagicMock(),
                mx.arange(4_096, dtype=mx.int32),
                cache,
                group=group,
                on_prefill_progress=None,
                distributed_prompt_progress_callback=None,
        ...
        assert set_prefill.call_args_list == [
            call(model, is_prefill=True),
            call(model, is_prefill=False),
        ]
        assert set_queue_sends.call_args_list == [
            call(model, queue_sends=True),
            call(model, queue_sends=False),
        ]

    def test_v4_generation_clear_resets_entries_not_counter(self):
        """add -> clear -> add yields gen 1 then 2, same instance ID."""
        prefix_cache = KVPrefixCache(None)
        instance_id = prefix_cache._instance_id

        # First add: generation 1
        prompt = mx.arange(12, dtype=mx.int32)
        cache = [_make_v4_cache(offset=12, pool_rows=3)]
        snapshot = CacheSnapshot(states=cache, token_count=12)
        prefix_cache.add_kv_cache(prompt, cache, [snapshot])
        ...
        # Clear resets entries but NOT the counter or instance ID
        prefix_cache.clear()
        assert len(prefix_cache._entry_generations) == 0
        assert prefix_cache._next_generation == 2  # not reset to 1
        assert prefix_cache._instance_id == instance_id  # same instance

        # Second add: generation 2 (not 1)
        prefix_cache.add_kv_cache(prompt, cache, [snapshot])
        assert prefix_cache._entry_generations == [2]

    def test_v4_generation_composite_id_differs_across_instances(self):
        """Composite IDs differ across cache instances even at same generation."""
        cache1 = KVPrefixCache(None)
        cache2 = KVPrefixCache(None)

        assert cache1._instance_id != cache2._instance_id

        prompt = mx.arange(12, dtype=mx.int32)
        cache = [_make_v4_cache(offset=12, pool_rows=3)]
        snapshot = CacheSnapshot(states=cache, token_count=12)

        cache1.add_kv_cache(prompt, cache, [snapshot])
        cache2.add_kv_cache(prompt, cache, [snapshot])

        # Both have generation 1 but different instance IDs -> different composite IDs
        assert cache1._entry_generations == [1]
        assert cache2._entry_generations == [1]
        composite1 = f"{cache1._instance_id}:1"
        composite2 = f"{cache2._instance_id}:1"
    ...
    def test_v4_generation_rollback_injection(self):
        ...
        assert len(prefix_cache.caches) == original_len
    ...
    def test_v4_generation_skipped_id_not_reused(self):
        ...
        # Add again: generation 2 was consumed by failed attempt, use generation 3
        prefix_cache.add_kv_cache(
            mx.arange(24, dtype=mx.int32),
            [_make_v4_cache(offset=24, pool_rows=6)],
            [snapshot],
    ...
    def test_prompt_observation_logs_alignment_boundary(self):
        ...
        with patch("exo.worker.engines.mlx.cache.logger.info") as log_info:
            prefix_cache.record_prompt_observation(
                base_prompt,
                source="uid=7",
            )
        ...
        recorded = [
            call.args[0]
            for call in log_info.call_args_list
            if call.args and "Prompt cache observation recorded" in call.args[0]
        ]
        assert len(recorded) == 1
        assert "source=uid=7" in recorded[0]
        assert "base=10" in recorded[0]
        ...
        with patch("exo.worker.engines.mlx.cache.logger.info") as log_info:
            prefix_cache._log_prompt_alignment(
                next_prompt,
                current_restore=6,
            )
        ...
        alignments = [
            call.args[0]
            for call in log_info.call_args_list
            if call.args and "Prompt cache alignment" in call.args[0]
        ]
        assert len(alignments) == 1
        assert "source=uid=7" in alignments[0]
        assert "base_match=8/11" in alignments[0]
        assert "base_length=10" in alignments[0]
        assert "base_remaining=2" in alignments[0]
        assert "prompt_remaining=3" in alignments[0]
        assert "current_restore=6" in alignments[0]
        assert "base_window=[4, 5, 6, 7, 8, 9]" in alignments[0]
        assert "prompt_window=[4, 5, 6, 7, 99, 100, 101]" in alignments[0]
        assert "analysis_ms=" in alignments[0]
...
class TestExperimentalBCapFour:
    ...
    def test_v4_selected_log_contains_entry_id_and_ordinal(self):
        ...
        query a non-exact match (97 tokens, only 95 shared) so the restore
        point falls on the middle snapshot (ordinal 1). Patch the Loguru
        logger directly since caplog does not capture it.
        """
        # Prompt is 120 tokens; query is 97 tokens sharing only 95 prefix.
        ...
        # Query with 95 shared prefix, target=95, restore at 80, ordinal=1.
        prefix_cache._snapshots = [
            [
                CacheSnapshot(
        ...
        with patch(
            "exo.worker.engines.mlx.cache.logger.info"
        ) as log_info:
        ...
        selected = [
            ...
            if call.args and call.args[0].startswith("KV cache selected:")
        ]
    ...
    def test_v4_selected_log_ordinal_minus_one_when_no_v4_snapshot(self):
        ...
        A non-V4 cache with _snapshots=[None] still produces a usable
        candidate, but no restore snap is set, so ordinal=-1.
        """
        query = mx.arange(100, dtype=mx.int32)
        ...
        # Use a plain KVCache (not V4) so no SSM snapshots are needed;
        # the entry is still restorable but best_restore_snap is None.
        prefix_cache.caches = [[KVCache()]]
